 ...
 * \param BitMapBuffer Pointer to caller-allocated memory for the bitmap itself. The base address of this buffer must be ULONG-aligned.
 * \param SizeOfBitMap Specifies the number of bits in the bitmap.
 * \remarks RtlInitializeBitMap must be called before any other RtlXxx routine that operates on a bitmap variable.
 * \sa https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-rtlinitializebitmap
 */
#ifdef VEIL_INLINE_RTL_BITMAP
FORCEINLINE
VOID
RtlInitializeBitMap(
 ...
 *
 * \param BitMapHeader A pointer to the RTL_BITMAP structure that describes the bitmap.
 ...
 * \param Handle A handle to the object. The handle must have SYNCHRONIZE access.
...
#define RTL_PTR_ADD(Base, Offset) ((PVOID)((PBYTE)(Base) + (Offset)))
